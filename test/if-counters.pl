...
sub out_interface {
	...
	printf "%2d  %-24s %10.1f %10.1f %10.1f %10.1f %s\n",
	$index,
	defined $descr ? $descr : '',
...
while (1) {
    $session->map_table ([$ifDescr,$ifInOctets,$ifOutOctets,$ifInUcastPkts,$ifOutUcastPkts,$locIfDescr],
			 \&out_interface);
